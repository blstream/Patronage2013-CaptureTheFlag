	...
	$scope.SignIn = function() {
		$.ajax({
			...
			error : function(errorData) {
				...
				//alert("Error: " + reason);
				$('#errorModal').find('.modal-body').html(reason);
				$('#errorModal').modal();
			}
		});
	}
		...
		$.ajax({
			...
			success : function(authData) {
				if(authData.error_code === 0) {
					...
					//alert(authData.error_description);
					$('#errorModal').find('.modal-body').html(authData.error_description);
					$('#errorModal').modal();
				}
			},
			error : function(errorData) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(typeof data.error != 'undefined') {
					...
					$('#errorModal').find('.modal-body').html(data.error_description);
					$('#errorModal').modal();
				} else {
